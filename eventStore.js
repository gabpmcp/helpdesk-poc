 ...
 */
export const fetchEventsByType = (supabaseClient) => 
  async (userId, eventType) => {
    ...
    if (error) {
      return Promise.reject(new Error(`Failed to fetch events by type: ${error.message}`));
    }
 ...
 */
export const fetchTicketEvents = (supabaseClient) => 
  async (ticketId) => {
    const { data, error } = await supabaseClient
      .from(EVENT_STORE_TABLE)
      .select('event_data')
      .or(`event_data->ticketId.eq.${ticketId}`)
      .order('timestamp', { ascending: true });
